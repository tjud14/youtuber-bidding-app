<script>
  ...
  function getDisplayValue(normalVal, hoverVal, isHovering) {
    if (isHovering) {
      return `${normalVal} → ${hoverVal}`;
    }
...
<div class="container mx-auto py-16 px-4">
  <h1 class="text-3xl font-bold text-center mb-8">Z-Value Transition Test</h1>
  <p class="text-center mb-4 max-w-2xl mx-auto">This test demonstrates Z-value changes between hover and normal states.</p>
    ...
    <div class="grid grid-cols-2 gap-4">
      ...
      <div class="p-2 rounded bg-gradient-to-br from-red-900/50 to-purple-900/50">
        ...
        <div class="font-mono text-lg">
          {isCardHovering ? getDisplayValue(baseZ, hoverBaseZ, true) : baseZ}px
        </div>
      </div>
    ...
    <div class="h-[400px] w-[300px]">
      ...
      >
        <svelte:fragment slot="default" let:getItemStyle let:isHovering>
          <!-- Update global state for debug display -->
          {#if isCardHovering !== isHovering}
            {@const dummy = (isCardHovering = isHovering)}
          {/if}
          ...
          >
            <div class="absolute top-2 left-2 right-2 text-center text-xs bg-black/40 backdrop-blur-sm rounded px-2 py-1">
              Base: {isHovering ? hoverBaseZ : baseZ}px
            </div>
          </div>
          ...
          >
            <div class="bg-black/20 backdrop-blur-sm h-40 w-40 rounded-full flex items-center justify-center">
              <span class="text-lg font-bold text-white">
                Middle ({isHovering ? hoverMiddleZ : middleZ}px)
              </span>
            </div>
          </div>
          ...
          >
            <div class="bg-white px-6 py-3 rounded-full shadow-xl relative">
              <span class="text-lg font-bold text-purple-700">
                Front ({isHovering ? hoverFrontZ : frontZ}px)
              </span>
          ...
          >
            <button class="bg-purple-600 text-white px-6 py-3 rounded-full font-bold shadow-xl relative">
              Top ({isHovering ? hoverTopZ : topZ}px)
              <div class={`absolute -top-1 -right-1 h-3 w-3 rounded-full transition-all ${isHovering ? 'bg-green-500' : 'bg-red-500'}`}></div>
            </button>
          </div>
        </svelte:fragment>
  ...
  <div class="mt-12 text-center text-sm text-white/60">
    ...
    <ul class="mt-2 space-y-1">
      ...
      <li><a href="/testcard2" class="underline">Direct DOM manipulation test</a></li>
    </ul>
  </div>
</div>
