...
<div class="container mx-auto py-12 px-4">
  ...
  <div class="bg-white/10 backdrop-blur-md p-4 rounded-lg mx-auto max-w-xl mb-10">
    ...
    <div class="grid grid-cols-2 gap-4">
      ...
      <div class="p-2 rounded bg-gradient-to-br from-red-900/50 to-purple-900/50">
        <div class="text-sm text-white/70">Background:</div>
        <div class="font-mono text-lg">
          {currentHoverCard ? `${zValues.background} → ${zValues.background * hoverScale}px` : `${zValues.background}px`}
        </div>
      </div>
      <div class="p-2 rounded bg-gradient-to-br from-indigo-900/50 to-blue-900/50">
        <div class="text-sm text-white/70">Image Layer:</div>
        <div class="font-mono text-lg">
          {currentHoverCard ? `${zValues.image} → ${zValues.image * hoverScale}px` : `${zValues.image}px`}
        </div>
      </div>
      <div class="p-2 rounded bg-gradient-to-br from-green-900/50 to-teal-900/50">
        <div class="text-sm text-white/70">Title:</div>
        <div class="font-mono text-lg">
          {currentHoverCard ? `${zValues.title} → ${zValues.title * hoverScale}px` : `${zValues.title}px`}
        </div>
      </div>
      <div class="p-2 rounded bg-gradient-to-br from-amber-900/50 to-yellow-900/50">
        <div class="text-sm text-white/70">Price:</div>
        <div class="font-mono text-lg">
          {currentHoverCard ? `${zValues.price} → ${zValues.price * hoverScale}px` : `${zValues.price}px`}
        </div>
      </div>
    </div>
  </div>
      ...
      <div class="h-[500px] card-container">
        ...
        >
          <svelte:fragment slot="default" let:isHovering let:getItemStyle>
            ...
            <!-- Z-value display for debugging -->
            <div class="absolute bottom-2 right-2 text-xs bg-black/40 text-white/70 px-2 py-1 rounded pointer-events-none">
  ...
  <div class="bg-white/10 backdrop-blur-sm p-6 rounded-xl max-w-3xl mx-auto">
    ...
    <ul class="list-disc pl-5 space-y-2">
      <li>Uses ReactiveExplicitZCard component with proper z-depth values</li>
      <li>Deep background elements: -100px (amplified to -400px on hover)</li>
      <li>Image layer: -40px (amplified to -160px on hover)</li>
      <li>Content elements: 20px to 80px (amplified to 80px-320px on hover)</li>
      <li>Interactive buttons: 100px (amplified to 400px on hover)</li>
      <li>The z-values dashboard shows real-time changes on hover</li>
    </ul>
    
    <div class="mt-6 p-4 bg-blue-900/30 rounded-lg">
      <h3 class="text-lg font-semibold mb-2">How to Fix 3D Clipping</h3>
      <p class="mb-2">The previous implementation had elements being cut off when they transformed outside their containers. This is fixed by:</p>
      <ul class="list-disc pl-5 space-y-1">
        <li>Removing <code>overflow: hidden</code> from card containers</li>
        <li>Setting <code>transform-style: preserve-3d</code> throughout the component hierarchy</li>
        <li>Using <code>overflow: visible</code> on all parent containers</li>
      </ul>
    </div>
  </div>
</div>
