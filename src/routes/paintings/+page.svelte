...
<div class="alaska-bg">
  <div class="container mx-auto min-h-screen p-8">
    <h1 class="mb-8 text-center text-4xl font-bold text-lime-200 gradient drop-shadow-lg">Paintings Gallery</h1>
          ...
          <CardContainer>
            <CardBody class="rounded-xl p-6 text-white">
              {#if item.timeRemaining && !item.timeRemaining.isExpired && item.timeRemaining.days < 1}
                <CardItem translateZ={60} class="text-center mb-2">
                  <span class="bg-purple-500/70 px-3 py-1 rounded-full text-sm">ENDING SOON</span>
                </CardItem>
              {/if}
              
              {#if item.images?.length > 0 && item.images[0].image}
                <CardItem as="div" translateZ={20} class="mb-4 h-64 w-full overflow-hidden rounded-lg">
                  <ResponsiveImage
              ...
              <CardItem as="p" translateZ={30} class="line-clamp-2 mb-4 text-center">
                {item.description}
              </CardItem>
              
              {#if item.timeRemaining}
                <CardItem translateZ={25} class="mb-5">
              ...
              <CardItem as="a" 
                ...
                translateZ={70} 
                class="block w-full bg-white text-purple-600 py-2 rounded-lg font-medium hover:bg-purple-50 text-center"
              >
                View Artwork
              </CardItem>
            </CardBody>
    ...
    {/if}
    <PastAuctions categoryCode="PAINT" categoryName=""
