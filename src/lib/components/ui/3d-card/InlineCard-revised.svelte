<script>
   ...
   export let initialZScale = 0.3; // How much of the Z effect to show initially (0-1)
   ...
   function getItemTransform(z, additionalTransform = '') {
     // Always apply some Z transform (initialZScale %), amplify on hover (hoverZScale)
     const zAmount = isHovering ? z * hoverZScale : z * initialZScale;
     
     // Make sure Z transformations are always visible by using a more pronounced minimum value
     const effectiveZAmount = Math.sign(z) * Math.max(Math.abs(zAmount), 0.5);
     
     return `translateZ(${effectiveZAmount}px) ${additionalTransform}`.trim();
   }
   </script>
